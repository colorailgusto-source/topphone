...
class CartService extends ChangeNotifier {
  ...
  double get total => _items.fold(
      0.0,
      (s, i) =>
          s + (i.product.prezzo + (i.variant?.prezzoExtra ?? 0)) * i.quantita);
  ...
  String? cannotAddReason(ProductModel product, {VariantModel? variant}) {
    if (_items.length >= maxItems) {
      return 'Carrello pieno (max $maxItems prodotti)';
    }
  ...
  Future<void> loadFromDb() async {
    ...
    try {
      ...
      for (final row in (data as List)) {
        ...
        final scadenza = DateTime.parse(scadenzaStr.contains('+')
  ...
  Future<bool> addItem(ProductModel product, int qty,
      {VariantModel? variant}) async {
    final userId = _client.auth.currentUser?.id;
    ...
    try {
      if (variant != null) {
        final result = await _client.rpc('decrement_stock_variante',
            params: {'variante_id': variant.id, 'qty': qty});
        int stockResult = -1;
        if (result is int) {
          stockResult = result;
        } else if (result is List && result.isNotEmpty)
          stockResult = result[0] as int;
        else if (result is Map) stockResult = (result.values.first as int);
        if (stockResult < 0) return false;
      } else {
        final result = await _client.rpc('decrement_stock_prodotto',
            params: {'prodotto_id': product.id, 'qty': qty});
      ...
      try {
        ...
        if (variant != null) {
          await _client.rpc('increment_stock_variante',
              params: {'variante_id': variant.id, 'qty': qty});
        } else {
          await _client.rpc('increment_stock_prodotto',
              params: {'prodotto_id': product.id, 'qty': qty});
        }
        return false;
      }
      ...
      await _client
          .from('carrelli')
          .update({'scadenza': scadenzaIso}).eq('utente_id', userId);
      ...
      Future.delayed(const Duration(minutes: 3), () async {
        ...
        if (_items.isNotEmpty) {
          ...
          if (firstItem.remaining.inMinutes <= 2 &&
              firstItem.remaining.inSeconds > 0) {
      ...
      Future.delayed(const Duration(minutes: 5), () async {
        await _removeExpiredItem(row['id'], product.id, qty,
  ...
  Future<void> _removeExpiredItem(String cartId, String productId, int qty,
    ...
    try {
      if (variantId != null) {
        await _client.rpc('increment_stock_variante',
            params: {'variante_id': variantId, 'qty': qty});
      } else {
        await _client.rpc('increment_stock_prodotto',
            params: {'prodotto_id': productId, 'qty': qty});
      }
  ...
  Future<void> removeItem(CartItemModel item) async {
    try {
      if (item.variant != null) {
        await _client.rpc('increment_stock_variante',
            params: {'variante_id': item.variant!.id, 'qty': item.quantita});
      } else {
        await _client.rpc('increment_stock_prodotto',
            params: {'prodotto_id': item.product.id, 'qty': item.quantita});
      }
      if (item.id != null) {
        await _client.from('carrelli').delete().eq('id', item.id!);
      }
    } catch (e) {
