...
class OrderService {
  ...
  Future<String> createOrder(
    ...
    for (final riga in righe) {
      await _client
          .from('righe_ordine')
          .insert({...riga, 'ordine_id': order['id']});
    }
    ...
    try {
      ...
      final telefono = profilo['telefono'] ?? '';
      final varianteLabel =
          righe.isNotEmpty ? (righe[0]['variante_label'] ?? '') : '';
      final prodottoNome = righe.isNotEmpty
          ? (righe[0]['nome_prodotto'] ?? '$nProdotti prodotto/i')
          : '$nProdotti prodotto/i';
