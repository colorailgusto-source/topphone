...
class OrderService {
  ...
  Future<List<OrderModel>> getUserOrdersDetailed(String userId) async {
    ...
    return (data as List).map((e) {
      ...
      final righe = (e['righe_ordine'] as List?)
          ?.map((r) => {
  ...
  Future<List<OrderModel>> getAllOrders() async {
    final data =
        await _client.from('ordini').select().order('data', ascending: false);
    return (data as List).map((e) => OrderModel.fromJson(e)).toList();
  }

  Future<String> createOrder(
      String userId, double totale, List<Map<String, dynamic>> righe,
      {String? note,
      String tipoConsegna = "ritiro",
      String stato = "ricevuto"}) async {
    ...
    try {
      ...
      await _client
          .rpc('marca_abbandoni_recuperati', params: {'p_user_id': userId});
    } catch (e) {
    ...
    final vl0 =
        righe.isNotEmpty ? ((righe[0]['variante_label'] ?? '') as String) : '';
    String pn = '$nProdotti prodotto/i';
    try {
      if (righe.isNotEmpty) {
        final pd = await _client
            .from('prodotti')
            .select('nome')
            .eq('id', righe[0]['prodotto_id'])
            .single();
        pn = pd['nome'] ?? '$nProdotti prodotto/i';
      }
    } catch (e) {
      debugPrint("order nome prodotto: $e");
    }
    final profCliente = await _client
        .from('profili')
        .select('nome, cognome')
        .eq('id', userId)
        .single();
    final nc =
        ((profCliente['nome'] ?? '') + ' ' + (profCliente['cognome'] ?? ''))
            .trim();
    ...
    try {
      ...
      for (final riga in righe) {
        ...
        label +=
            ' x${riga['quantita']} - \u20AC${(riga['prezzo'] as num).toStringAsFixed(2)}';
  ...
  Future<void> updateOrderStatus(String id, String stato,
