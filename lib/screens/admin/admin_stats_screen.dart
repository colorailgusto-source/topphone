...
class _AdminStatsScreenState extends State<AdminStatsScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      final inizioSettimana = inizioOggi.subtract(Duration(days: inizioOggi.weekday - 1));
      final inizioMese = DateTime(now.year, now.month, 1);

      final ordini = await _client.from('ordini').select('totale, stato, data')
        .neq('stato', 'annullato')
      ...
      for (final o in ordini) {
        ...
        if (data.isAfter(inizioSettimana) || data.isAtSameMomentAs(inizioSettimana)) {
          settimana += totale;
          nSettimana++;
        }
        ...
        switch (o['stato']) {
          case 'ricevuto': ricevuti++; break;
          case 'spedito': spediti++; break;
          case 'consegnato': consegnati++; break;
        }
      }
      ...
      for (final r in righe) {
        ...
        if (venditeMap.containsKey(pid)) {
          venditeMap[pid]!['vendite'] = (venditeMap[pid]!['vendite'] as int) + qty;
        } else {
      ...
      final giorni = giorniMap.entries.map((e) => {
        'data': e.key,
        'label': DateFormat('EEE', 'it_IT').format(DateTime.parse(e.key)),
        'totale': e.value,
      }).toList();
        ...
        : RefreshIndicator(
            ...
            child: SingleChildScrollView(
              ...
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                // VENDITE PER PERIODO
                const Text('Vendite', style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold, fontFamily: 'Poppins')),
                const SizedBox(height: 12),
                Row(children: [
                  Expanded(child: _periodCard('Oggi', _venditeOggi, _ordiniOggi, Colors.green)),
                  const SizedBox(width: 8),
                  Expanded(child: _periodCard('Settimana', _venditeSettimana, _ordiniSettimana, Colors.blue)),
                  const SizedBox(width: 8),
                  Expanded(child: _periodCard('Mese', _venditeMese, _ordiniMese, Colors.purple)),
                ]),

                const SizedBox(height: 20),

                // GRAFICO ULTIMI 7 GIORNI
                const Text('Ultimi 7 giorni', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold, fontFamily: 'Poppins')),
                const SizedBox(height: 12),
                _buildChart(),
                ...
                // STATS GENERALI
                const Text('Riepilogo Totale', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold, fontFamily: 'Poppins')),
                const SizedBox(height: 12),
                Row(children: [
                  Expanded(child: _statCard('Fatturato', '€${_totaleVendite.toStringAsFixed(0)}', Icons.euro, Colors.green)),
                  const SizedBox(width: 12),
                  Expanded(child: _statCard('Ordini', '$_totaleOrdini', Icons.receipt_long, Colors.blue)),
                ]),
                const SizedBox(height: 12),
                Row(children: [
                ...
                // PRODOTTI PIU VENDUTI
                const Text('Prodotti Piu Venduti', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold, fontFamily: 'Poppins')),
                const SizedBox(height: 12),
                ..._topProducts.take(10).map((p) => Card(child: ListTile(
  ...
  Widget _periodCard(String label, double vendite, int ordini, Color color) {
    return Container(
      ...
      child: Column(children: [
        ...
        Text('€${vendite.toStringAsFixed(0)}', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w800, color: color)),
        const SizedBox(height: 2),
        Text('$ordini ordini', style: const TextStyle(fontSize: 10, color: AppTheme.grey)),
      ]),
    );
  }
  ...
  Widget _buildChart() {
    ...
    final maxVal = _ultimi7Giorni.fold<double>(0, (prev, e) => (e['totale'] as double) > prev ? (e['totale'] as double) : prev);
    ...
    return Container(
      ...
      child: Column(children: [
              ...
              return Expanded(
                child: Padding(
                  ...
                  child: Column(mainAxisAlignment: MainAxisAlignment.end, children: [
                    if (val > 0) Text('€${val.toStringAsFixed(0)}', style: const TextStyle(fontSize: 8, fontWeight: FontWeight.w600, color: AppTheme.primary)),
                    const SizedBox(height: 4),
                    Container(
                      height: height < 4 && val > 0 ? 4 : height,
                      decoration: BoxDecoration(
                        color: val > 0 ? AppTheme.primary : Colors.grey.shade200,
        ...
        Row(
          children: _ultimi7Giorni.map((day) => Expanded(
            child: Text(
