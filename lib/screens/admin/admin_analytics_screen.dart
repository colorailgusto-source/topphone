...
class _AdminAnalyticsScreenState extends State<AdminAnalyticsScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      final views = await _client
          .from('analytics')
          .select('prodotto_id, prodotti(nome, marca)')
          .eq('evento', 'view');
      ...
      final allAnalytics =
          await _client.from('analytics').select('evento, created_at');

      // Conta per prodotto
      final viewMap = <String, Map<String, dynamic>>{};
      for (final v in views) {
        final pid = v['prodotto_id'];
        if (!viewMap.containsKey(pid)) {
          viewMap[pid] = {
            'nome': v['prodotti']?['nome'] ?? 'Sconosciuto',
            'marca': v['prodotti']?['marca'] ?? '',
            'count': 0
          };
        }
      ...
      final sortedViews = viewMap.values.toList()
        ..sort((a, b) => (b['count'] as int).compareTo(a['count'] as int));
      final sortedCart = cartMap.values.toList()
        ..sort((a, b) => (b['count'] as int).compareTo(a['count'] as int));
          ...
          : RefreshIndicator(
              ...
              child: ListView(
                ...
                children: [
                  ...
                  Row(children: [
                    _statCard(
                        '👁️ Visualizzazioni', '$_totalViews', Colors.blue),
                  ...
                  ..._topViews.asMap().entries.map((e) => _rankTile(
                      e.key + 1,
                      e.value['marca'],
                      e.value['nome'],
                      e.value['count'],
                  ...
                  ..._topCart.asMap().entries.map((e) => _rankTile(
                      e.key + 1,
                      e.value['marca'],
                      e.value['nome'],
                      e.value['count'],
                  ...
                  if (_orePunta.isNotEmpty) ...[
                    ...
                    Container(
                      ...
                      child: Column(
                          children: (_orePunta.entries.toList()
                                ..sort((a, b) => b.value.compareTo(a.value)))
                              .take(5)
                              .map((e) {
                        final max =
                            _orePunta.values.reduce((a, b) => a > b ? a : b);
                        return Padding(
  ...
  Widget _convRate(String label, String value, Color color) =>
      Column(children: [
        Text('$value%',
            style: TextStyle(
  ...
  Widget _rankTile(
          int rank, String marca, String nome, int count, Color color) =>
      Container(
