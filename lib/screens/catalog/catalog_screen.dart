...
class _CatalogScreenState extends State<CatalogScreen> {
  ...
  Future<void> _load() async {
    ...
    if (mounted) {
      setState(() {
        ...
        _filtered = _selectedCategoria == 'Tutti'
            ? p
            : p
                .where((prod) =>
                    prod.marca.toLowerCase() ==
      ...
      _filtered = _all.where((p) {
        final matchSearch =
            p.nome.toLowerCase().contains(_search.toLowerCase()) ||
                p.marca.toLowerCase().contains(_search.toLowerCase());
        final matchCat = _selectedCategoria == 'Tutti' ||
            p.marca.toLowerCase() == _selectedCategoria.toLowerCase();
        final matchPrezzo = p.prezzo <= _maxPrezzo;
        final variants = _variantsMap[p.id] ?? [];
        final matchMemoria = _selectedMemoria.isEmpty ||
            variants.any((v) => v['memoria']?.toString() == _selectedMemoria);
        final matchRam = _selectedRam.isEmpty ||
            variants.any((v) => v['ram']?.toString() == _selectedRam);
        return matchSearch &&
            matchCat &&
            matchPrezzo &&
            matchMemoria &&
        ...
        builder: (ctx, setS) => Padding(
          ...
          child: Column(
              ...
              children: [
                ...
                Text('Prezzo massimo: \u20AC${tempPrezzo.toStringAsFixed(0)}',
                ...
                SizedBox(
                    ...
                    child: ElevatedButton(
                      onPressed: () {
                        setState(() {
                          _maxPrezzo = tempPrezzo;
                          _selectedMemoria = tempMemoria;
                          _selectedRam = tempRam;
                        });
                        _filter();
                        Navigator.pop(ctx);
                      },
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: const GradientAppBar(title: 'Catalogo'),
      body: Column(children: [
            ...
            children: [
              Padding(
                  ...
                  child: FilterChip(
                    ...
                    onSelected: (_) {
                      setState(() => _selectedCategoria = 'Tutti');
                      _filter();
                    },
                    selectedColor: AppTheme.primary,
                    labelStyle: TextStyle(
                        color: _selectedCategoria == 'Tutti'
                            ? Colors.white
                            : AppTheme.textDark),
                  )),
              ..._categorie.map((cat) => Padding(
                  padding: const EdgeInsets.only(right: 8),
                  child: FilterChip(
                    label: Text(cat['nome']),
                    selected: _selectedCategoria == cat['nome'],
                    onSelected: (_) {
                      setState(() => _selectedCategoria = cat['nome']);
                      _filter();
                    },
                    selectedColor: AppTheme.primary,
                    labelStyle: TextStyle(
                        color: _selectedCategoria == cat['nome']
                            ? Colors.white
                            : AppTheme.textDark),
                  ))),
            ],
        ...
        Padding(
          ...
          child: Row(children: [
            Expanded(
                child: _selectedMemoria.isNotEmpty ||
                    ...
                    ? Wrap(spacing: 6, children: [
                        if (_maxPrezzo < 1500)
                          Chip(
                              label: Text(
                                  'Max \u20AC${_maxPrezzo.toStringAsFixed(0)}'),
                              onDeleted: () {
                                setState(() => _maxPrezzo = 1500);
                                _filter();
                              },
                              deleteIconColor: Colors.red),
                        if (_selectedMemoria.isNotEmpty)
                          Chip(
                              label: Text(_selectedMemoria),
                              onDeleted: () {
                                setState(() => _selectedMemoria = '');
                                _filter();
                              },
                              deleteIconColor: Colors.red),
                        if (_selectedRam.isNotEmpty)
                          Chip(
                              label: Text(_selectedRam),
                              onDeleted: () {
                                setState(() => _selectedRam = '');
                                _filter();
                              },
                              deleteIconColor: Colors.red),
                      ])
              ...
              : _filtered.isEmpty
                  ...
                  : GridView.builder(
                      ...
                      itemBuilder: (c, i) => ProductCard(
                          ...
                          variants: _variantsMap[_filtered[i].id],
                          showDisponibile: false),
                    ),
        ),
      ]),
    );
