...
class _CompareScreenState extends State<CompareScreen> {
  ...
  Future<void> _load() async {
    ...
    if (widget.productId2 != null) {
      p2 = await _client.from('prodotti').select().eq('id', widget.productId2!).single();
      final v2data = await _client.from('varianti_prodotto').select().eq('prodotto_id', widget.productId2!).order('prezzo_extra').limit(1);
  ...
  Future<void> _selectProduct2(String id) async {
    ...
    for (final v in v2data) { final k = (v['ram'] ?? '') + '/' + (v['memoria'] ?? ''); uniqueVarianti.putIfAbsent(k, () => v); }
  ...
  Color _compareColor(dynamic val1, dynamic val2, bool higherIsBetter) {
    ...
    return higherIsBetter ? (v1 > v2 ? Colors.green : Colors.red) : (v1 < v2 ? Colors.green : Colors.red);
  }
          ...
          : Column(children: [
                ...
                child: SafeArea(
                  ...
                  child: Padding(
                    ...
                    child: Row(children: [
                      ...
                      const Text('Confronta Prodotti', style: TextStyle(color: Colors.white, fontSize: 18, fontWeight: FontWeight.w700, fontFamily: 'Poppins')),
                    ]),
                  ),
                ),
              ),
                ...
                child: Row(children: [
                  ...
                  Padding(
                    ...
                    child: Container(
                      ...
                      child: const Center(child: Text('VS', style: TextStyle(color: Colors.white, fontSize: 11, fontWeight: FontWeight.w800))),
                    ),
                  ),
                  // Prodotto 2
                  Expanded(child: GestureDetector(
                    ...
                    child: Container(
                      padding: const EdgeInsets.all(12),
                      decoration: BoxDecoration(
                        color: _p2 != null ? Colors.orange.withValues(alpha: 0.08) : Colors.grey.shade100,
                        borderRadius: BorderRadius.circular(16),
                        border: Border.all(color: _p2 != null ? Colors.orange.withValues(alpha: 0.3) : Colors.grey.shade300),
                      ),
                      child: _p2 == null
                        ? Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                            ...
                            const SizedBox(height: 8),
                            Text('Seleziona\nprodotto', textAlign: TextAlign.center, style: TextStyle(color: Colors.grey.shade400, fontSize: 12)),
                            const SizedBox(height: 10),
                          ])
                        : Column(children: [
                            GestureDetector(
              ...
              if (_p2 != null) Expanded(
                ...
                child: Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                  ...
                  const SizedBox(height: 16),
                  Text('Seleziona un prodotto\nda confrontare', textAlign: TextAlign.center, style: TextStyle(color: Colors.grey.shade400, fontSize: 16, fontFamily: 'Poppins')),
  ...
  Widget _buildSpecCard(String label, String val1, String val2, dynamic numVal1, dynamic numVal2, bool higherIsBetter) {
    final color1 = numVal1 != null && numVal2 != null ? _compareColor(numVal1, numVal2, higherIsBetter) : Colors.grey;
    final color2 = numVal1 != null && numVal2 != null ? _compareColor(numVal2, numVal1, higherIsBetter) : Colors.grey;
    return Container(
      ...
      child: Padding(
        ...
        child: Row(children: [
          Expanded(child: Text(val1, textAlign: TextAlign.center, style: TextStyle(fontSize: 15, fontWeight: FontWeight.w700, color: color1))),
          Container(
            width: 80,
            alignment: Alignment.center,
            child: Text(label, textAlign: TextAlign.center, style: const TextStyle(fontSize: 11, color: AppTheme.grey, fontWeight: FontWeight.w600)),
          ),
          Expanded(child: Text(val2, textAlign: TextAlign.center, style: TextStyle(fontSize: 15, fontWeight: FontWeight.w700, color: color2))),
        ]),
      ),
    );
  }
