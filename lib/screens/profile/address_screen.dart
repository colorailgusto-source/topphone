...
class _AddressScreenState extends State<AddressScreen> {
    ...
    try {
      ...
      if (addresses.isEmpty) {
        ...
        if (profilo != null && (profilo['via'] ?? '').isNotEmpty) {
          ...
          if (mounted) {
            setState(() {
              _addresses = List<Map<String, dynamic>>.from(newData);
              _loading = false;
            });
          }
          return;
  ...
  Future<void> _loadCittaFromCap(
      String cap,
      StateSetter setS,
      TextEditingController cittaCtrl,
      TextEditingController provinciaCtrl) async {
    ...
    try {
      ...
      if (res.statusCode == 200) {
        ...
        if (places.length == 1) {
          setS(() {
            cittaCtrl.text = places[0]['citta'] ?? '';
            provinciaCtrl.text = places[0]['provincia'] ?? '';
          });
        } else {
          if (!mounted) return;
          showDialog(
                  ...
                  child: Column(
                    ...
                    children: places
                        .map<Widget>((p) => ListTile(
                              ...
                              onTap: () {
                                setS(() {
                                  cittaCtrl.text = p['citta'] ?? '';
                                  provinciaCtrl.text = p['provincia'] ?? '';
                                });
                                Navigator.pop(ctx);
                              },
                            ))
  ...
  Future<void> _deleteAddress(Map<String, dynamic> address) async {
    final confirm = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Text(
            'Vuoi eliminare l\'indirizzo di ${address['nome_destinatario']}?'),
        actions: [
          TextButton(
  ...
  Future<void> _togglePredefinito(Map<String, dynamic> address) async {
    ...
    if (!isPredefinito) {
      ...
      await _client
          .from('indirizzi')
          .update({'predefinito': false}).eq('utente_id', userId);
      await _client
          .from('indirizzi')
          .update({'predefinito': true}).eq('id', address['id']);
    } else {
      // Rimuovi predefinito
      await _client
          .from('indirizzi')
          .update({'predefinito': false}).eq('id', address['id']);
    }
    _load();
  }
  ...
  void _showForm({Map<String, dynamic>? address}) {
    final nomeCtrl =
        TextEditingController(text: address?['nome_destinatario'] ?? '');
    final telefonoCtrl =
        TextEditingController(text: address?['telefono'] ?? '');
    ...
    final provinciaCtrl =
        TextEditingController(text: address?['provincia'] ?? '');
            ...
            child: Form(
              ...
              child: Column(mainAxisSize: MainAxisSize.min, children: [
                ...
                Row(children: [
                  Expanded(
                      ...
                      child: TextFormField(
                        ...
                        onChanged: (v) => _loadCittaFromCap(
                            v, setS, cittaCtrl, provinciaCtrl),
                        decoration: const InputDecoration(
                            labelText: 'CAP *', hintText: '80059'),
                        validator: (v) =>
                            v!.trim().length != 5 ? 'CAP non valido' : null,
                      )),
                  const SizedBox(width: 8),
                  Expanded(
                      flex: 2,
                ...
                TextFormField(
                  ...
                  decoration: const InputDecoration(
                      labelText: 'Provincia (es. NA) *',
                      prefixIcon: Icon(Icons.map_outlined)),
                  validator: (v) => v!.trim().length != 2 ? 'Es: NA' : null,
                ),
                const SizedBox(height: 8),
                SwitchListTile(
                  title: const Text('Imposta come predefinito',
                      style: TextStyle(fontFamily: 'Poppins')),
                  subtitle: const Text('Usato per default al checkout',
                ...
                SizedBox(
                    ...
                    child: ElevatedButton(
                      onPressed: () async {
                        ...
                        if (userId == null) return;
                        if (predefinito) {
                          await _client.from('indirizzi').update(
                              {'predefinito': false}).eq('utente_id', userId);
                        }
                        final indirizzo =
                            '${viaCtrl.text.trim()} ${civicoCtrl.text.trim()}, ${capCtrl.text.trim()} ${cittaCtrl.text.trim()} (${provinciaCtrl.text.trim().toUpperCase()})';
                        final data = {
                          'utente_id': userId,
                ...
                : _addresses.isEmpty
                    ? Center(
                        child: Column(
                            ...
                            const SizedBox(height: 16),
                            const Text('Nessun indirizzo salvato',
                                style: TextStyle(
                                    color: AppTheme.grey,
                                    fontSize: 16,
                                    fontFamily: 'Poppins')),
