...
class _CashbackScreenState extends State<CashbackScreen> {
  ...
  Future<void> _genera(int puntiRichiesti) async {
    ...
    if (!integrityOk) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
            content: Text('Verifica sicurezza fallita.'),
    ...
    try {
      ...
      if (codice != null) {
        ...
        if (mounted) {
          showDialog(
              ...
              builder: (_) => AlertDialog(
                    ...
                    title: const Text('🎉 Coupon Generato!',
                        style: TextStyle(
                            fontFamily: 'Poppins',
                    ...
                    content: Column(mainAxisSize: MainAxisSize.min, children: [
                      ...
                      const Text('Valido 30 giorni solo su spedizione',
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
              content: Text('Punti insufficienti'),
              backgroundColor: Colors.red));
        }
      }
    } catch (e) {
          ...
          child: Column(children: [
            ...
            Container(
              ...
              child: Column(children: [
                ...
                const SizedBox(height: 8),
                Text('Totale acquistati: $totali telefoni',
                    style:
            ...
            Container(
              ...
              child: const Column(
                  ...
                  children: [
                    ...
                    SizedBox(height: 8),
                    Text(
                        '📱 Ogni telefono acquistato e consegnato = 1 punto\n🎁 Accumula punti e riscatta sconti\n📦 I coupon sono validi solo su spedizione\n⏰ I coupon scadono dopo 30 giorni',
                        style: TextStyle(
  ...
  Widget _progressBar(int disponibili, int target, String label) {
    ...
    return Row(children: [
      ...
      const SizedBox(width: 8),
      Icon(disponibili >= target ? Icons.check_circle : Icons.lock,
          color: disponibili >= target ? Colors.greenAccent : Colors.white38,
          size: 20),
    ]);
  }

  Widget _premioCard(int punti, double valore, int disponibili,
      Map<String, dynamic> puntiData) {
    final giaRiscattato =
        punti != 10 && _pointsService.isPremioRiscattato(puntiData, punti);
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border:
            abilitato ? Border.all(color: AppTheme.primary, width: 1.5) : null,
      ),
      child: Row(children: [
        ...
        Expanded(
            child:
                Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text('$punti punti → \u20AC${valore.toStringAsFixed(0)} sconto',
              style: const TextStyle(
                  fontWeight: FontWeight.bold, fontFamily: 'Poppins')),
          Text(
              abilitato
                  ? 'Puoi riscattare!'
                  : 'Ti mancano ${punti - disponibili} punti',
              style: TextStyle(
                  fontSize: 12, color: abilitato ? Colors.green : Colors.grey)),
        ])),
        ElevatedButton(
      ...
      child: Row(children: [
        ...
        Expanded(
          ...
          Text(c['codice'],
              style: TextStyle(
                  fontWeight: FontWeight.bold,
                  fontFamily: 'Poppins',
                  color: usato || scaduto ? Colors.grey : Colors.black)),
          Text(
              '\u20AC${(c['valore'] as num).toStringAsFixed(0)} sconto • ${usato ? 'Usato' : scaduto ? 'Scaduto' : 'Valido fino al ${scadenza.day}/${scadenza.month}/${scadenza.year}'}',
        ...
        if (!usato && !scaduto)
          IconButton(
              ...
              onPressed: () {
                Clipboard.setData(ClipboardData(text: c["codice"]));
                ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
                    content: Text("Codice copiato!"),
                    backgroundColor: Colors.green,
                    duration: Duration(seconds: 1)));
