...
class _CartScreenState extends State<CartScreen> {
  ...
  void initState() {
    ...
    _timer = Timer.periodic(const Duration(seconds: 1), (_) {
      if (mounted) {
        setState(() {});
        // Se qualche item e scaduto, ricarica il carrello
  ...
  Future<void> _doCheckout(BuildContext sheetCtx, StateSetter setS,
      {String metodoPagamento = 'carta'}) async {
    final cart = context.read<CartService>();
    final auth = context.read<AuthService>();
    final userId = auth.currentUser?.id;
    if (userId == null) return;
    // Play Integrity - verifica autenticita app solo in produzione
    final integrityOk = await IntegrityService.verifica();
    if (!integrityOk) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
            content: Text('Verifica sicurezza fallita. Aggiorna l app.'),
    ...
    try {
        ...
        if (_nomeSpedizioneCtrl.text.trim().isEmpty ||
          ...
          showDialog(
              ...
              builder: (ctx) => AlertDialog(
                    ...
                    content: const Text(
                        "Compila tutti i campi obbligatori per la spedizione!"),
                    actions: [
                      ElevatedButton(
      ...
      if (_tipoConsegna == 'spedizione') {
        final righeStripe = cart.items
            .map((i) => {
                  "prodotto_id": i.product.id,
                  "quantita": i.quantita,
                  "prezzo": i.product.prezzo + (i.variant?.prezzoExtra ?? 0),
                  "variante_id": i.variant?.id,
                  "variante_label": i.variant != null
                      ? "${i.variant!.ram} ${i.variant!.memoria} ${i.variant!.colore}"
                          .trim()
                      : null,
                })
            .toList();
        ...
        await prefs.setDouble(
            'pending_total', cart.total + _costoSpedizione - _scontoCoupon);
        ...
        final prezzoProdotti = metodoPagamento == 'klarna'
            ? cart.total * (1 + _klarnaMarkup / 100)
            : cart.total;
        final totaleDaPagare =
            (prezzoProdotti + _costoSpedizione - _scontoCoupon)
                .clamp(0.0, double.infinity);
        final payResult = await StripeService.openPaymentSheet(
          totaleDaPagare,
          userId: userId,
        ...
        if (payResult == 'redirected') {
          if (mounted) setState(() => _ordering = false);
          if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
                content: Text('Completa il pagamento nella pagina aperta.'),
        ...
        if (metodoPagamento == 'scalapay') {
          ...
          if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
                content: Text(
                    'Completa il pagamento ScalaPay nel browser. Il tuo ordine apparira in Ordini una volta confermato.'),
                backgroundColor: Colors.orange,
                duration: Duration(seconds: 6)));
          }
          return;
        ...
        // Il webhook Stripe gestisce la cancellazione del carrello.
        // Evita il doppio decremento dello stock prima della RPC.
        cart.clear();

        if (mounted) setState(() => _ordering = false);
      } else {
        // Fix: verifica carrello ancora valido (non scaduto)
        final carrelliValidi = await Supabase.instance.client
            .from('carrelli')
            .select('id')
            .eq('utente_id', userId)
        ...
        await _orderService.createOrder(userId, cart.total, righe,
            note: note, tipoConsegna: _tipoConsegna);
        await cart.clearAfterOrder();
      }
      if (mounted) {
        if (_tipoConsegna != 'spedizione') {
          ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
              content: Text('Ordine confermato! Ti aspettiamo in negozio.'),
      ...
      if (mounted) {
        showDialog(
            ...
            builder: (ctx) => AlertDialog(
                  ...
                  content: const Text(
                      'Il prodotto non e piu disponibile. Il tuo carrello verra svuotato.'),
                  actions: [
                    ElevatedButton(
                        onPressed: () async {
                          Navigator.pop(ctx);
                          await context.read<CartService>().clearAfterOrder();
                        },
                        child: const Text('OK'))
                  ],
                ));
  ...
  Future<void> _loadConfig() async {
    try {
      final data = await Supabase.instance.client
          .from('app_config')
          .select(
              'ritiro_attivo, spedizione_attiva, klarna_attivo, klarna_markup, costo_spedizione, scalapay_attivo, scalapay_markup')
          .eq('id', 'config')
          .single();
      if (mounted) {
        setState(() {
            ...
            child: SingleChildScrollView(
                ...
                child: Column(mainAxisSize: MainAxisSize.min, children: [
                  ...
                  Row(mainAxisAlignment: MainAxisAlignment.center, children: [
                    Column(children: [
                      ChoiceChip(
                        ...
                        onSelected: _ritiroAttivo
                            ? (_) => setS(() {
                                  _tipoConsegna = 'ritiro';
                                  _scontoCoupon = 0;
                                  _couponValidato = null;
                                  _couponCtrl.clear();
                                })
                            : null,
                    ...
                    Column(children: [
                      ChoiceChip(
                        ...
                        onSelected: _spedizioneAttiva
                            ? (_) => setS(() => _tipoConsegna = 'spedizione')
                            : null,
                        selectedColor: AppTheme.primary,
                        disabledColor: Colors.grey.shade200,
                        labelStyle: TextStyle(
                  ...
                  if (_tipoConsegna == 'ritiro') ...[
                    const Text(
                        'Top Phone Torre - ${AppConfig.shopStreet}, ${AppConfig.shopCity}',
                      ...
                      child: const Column(
                          ...
                          children: [
                            Row(children: [
                              ...
                              Expanded(
                                  child: Text(
                                      'Lun-Sab: 09:30-13:30 / 16:30-20:00',
                            ...
                            Row(children: [
                              Icon(Icons.payment,
                                  size: 16, color: AppTheme.primary),
                              SizedBox(width: 8),
                              Text("Pagamento in sede al ritiro",
                                  style: TextStyle(fontSize: 13))
                            ]),
                          ]),
                    ),
                    ...
                    TextField(
                        controller: _nomeSpedizioneCtrl,
                        decoration: const InputDecoration(
                            labelText: 'Nome e Cognome *',
                            prefixIcon: Icon(Icons.person_outlined))),
                    const SizedBox(height: 8),
                    TextField(
                        controller: _telefonoSpedCtrl,
                    ...
                    Row(children: [
                      ...
                      Expanded(
                          ...
                          child: TextField(
                              ...
                              decoration:
                                  const InputDecoration(labelText: 'Citta *'))),
                    ]),
                    const Divider(height: 24),
                  ],
                  const SizedBox(height: 8),
                  TextField(
                      ...
                      maxLines: 2,
                      decoration: const InputDecoration(
                          labelText: 'Note per il negozio (opzionale)',
                  ...
                  if (_tipoConsegna == 'spedizione') ...[
                    ...
                    Row(children: [
                      Expanded(
                        ...
                        decoration: InputDecoration(
                          ...
                          suffixIcon: _couponValidato != null
                      ...
                      ElevatedButton(
                        onPressed: () async {
                          ...
                          if (userId == null || _couponCtrl.text.trim().isEmpty) {
                            return;
                          }
                          final ps = PointsService();
                          final sconto = await ps.verificaCoupon(
                              _couponCtrl.text.trim(), userId);
                          if (!sheetCtx.mounted) return;
                          if (sconto != null) {
                            setS(() {
                              _scontoCoupon = sconto;
                              _couponValidato = _couponCtrl.text.trim();
                            });
                            ScaffoldMessenger.of(sheetCtx).showSnackBar(SnackBar(
                                content: Text(
                                    'Coupon applicato! \u20AC${sconto.toStringAsFixed(0)} di sconto'),
                                backgroundColor: Colors.green));
                          } else {
                            setS(() {
                              _scontoCoupon = 0;
                            ...
                            ScaffoldMessenger.of(sheetCtx).showSnackBar(
                                const SnackBar(
                                    content:
                                        Text('Coupon non valido o scaduto'),
                                    backgroundColor: Colors.red));
                          }
                        },
                    ...
                    if (_scontoCoupon > 0)
                      Padding(
                        ...
                        child: Text(
                            'Sconto applicato: \u20AC${_scontoCoupon.toStringAsFixed(0)}',
                                ...
                                children: [
                                  const Text('Spese spedizione:',
                                      style: TextStyle(
                                          color: AppTheme.grey, fontSize: 13)),
                                  Text(
                                      '\u20AC${_costoSpedizione.toStringAsFixed(2)}',
                                      style: const TextStyle(
                                          color: Colors.orange,
                                          fontSize: 13,
                                          fontWeight: FontWeight.bold)),
                            ...
                            if (_scontoCoupon > 0) ...[
                              ...
                              Row(
                                  mainAxisAlignment:
                                      MainAxisAlignment.spaceBetween,
                                  children: [
                                    const Text('Coupon sconto:',
                                        style: TextStyle(
                                            color: Colors.green, fontSize: 13)),
                                    Text(
                                        '-\u20AC${_scontoCoupon.toStringAsFixed(2)}',
                                        style: const TextStyle(
                                            color: Colors.green,
                                            fontSize: 13,
                                            fontWeight: FontWeight.bold)),
                              ...
                              children: [
                                Text(
                                    ...
                                    style: const TextStyle(
                                        fontWeight: FontWeight.bold)),
                                Text(
                                    "\u20AC${_tipoConsegna == "spedizione" ? (cart.total + _costoSpedizione - _scontoCoupon).clamp(0.0, double.infinity).toStringAsFixed(2) : cart.total.toStringAsFixed(2)}",
                                    style: const TextStyle(
                                        fontSize: 18,
                                        fontWeight: FontWeight.bold,
                                        color: AppTheme.primary)),
                  ...
                  if (_tipoConsegna == 'spedizione' &&
                    ...
                    Row(children: [
                      Expanded(
                        ...
                        onTap: () {
                          if (_ordering) return;
                          _doCheckout(sheetCtx, setS, metodoPagamento: 'carta');
                        },
                        child: Container(
                          ...
                          child: Container(
                            ...
                            child: Column(
                                ...
                                children: [
                                  ...
                                  const SizedBox(height: 2),
                                  Text(
                                      '\u20AC${(cart.total * (1 + _klarnaMarkup / 100) + _costoSpedizione - _scontoCoupon).clamp(0.0, double.infinity).toStringAsFixed(2)}',
                                      style: const TextStyle(
                                          color: Colors.black,
                                          fontWeight: FontWeight.w700,
                                          fontSize: 14)),
                          ...
                          child: Container(
                            ...
                            child: Column(
                                ...
                                children: [
                                  ...
                                  const SizedBox(height: 2),
                                  Text(
                                      '\u20AC${(cart.total * (1 + _scalapayMarkup / 100) + _costoSpedizione - _scontoCoupon).clamp(0.0, double.infinity).toStringAsFixed(2)}',
                                      style: const TextStyle(
                                          color: Colors.black,
                                          fontWeight: FontWeight.w700,
                                          fontSize: 14)),
                    ...
                    Center(
                        child: Text(
                            _klarnaAttivo && _scalapayAttivo
                                ? 'Info: Con Klarna o ScalaPay il prezzo del prodotto aumenta per i costi del servizio a rate'
                                : _klarnaAttivo
                                    ? 'Info: Con Klarna il prezzo del prodotto aumenta del ${_klarnaMarkup.toStringAsFixed(0)}% per i costi del servizio a rate'
                                    : 'Info: Con ScalaPay il prezzo del prodotto aumenta del ${_scalapayMarkup.toStringAsFixed(0)}% per i costi del servizio a rate',
                            textAlign: TextAlign.center,
                            style: const TextStyle(
                                fontSize: 11,
                                color: AppTheme.grey,
  ...
  Widget build(BuildContext context) {
        ...
        body: Center(
          child: Padding(
            ...
            child:
              ...
              const Text('Accedi per acquistare',
                  style: TextStyle(
                      fontSize: 20,
                      fontWeight: FontWeight.w700,
                      fontFamily: 'Poppins')),
              const SizedBox(height: 12),
              const Text(
                  'Registrati gratuitamente per aggiungere prodotti al carrello e acquistare online!',
                  textAlign: TextAlign.center,
    ...
    return Scaffold(
      appBar: const GradientAppBar(title: 'Carrello'),
      body: items.isEmpty
          ? const Center(
              child: Column(
                  ...
                  return Card(
                    ...
                    leading: item.product.immagine.isNotEmpty
                        ? ClipRRect(
                            ...
                            child: Image.network(item.product.immagine,
                                width: 50,
                                height: 50,
                                fit: BoxFit.contain,
                    ...
                    subtitle: Column(
                        ...
                        children: [
                          if ((item.variant != null
                                  ? "${item.variant!.ram} ${item.variant!.memoria} ${item.variant!.colore}"
                                      .trim()
                                  : "")
                              .isNotEmpty)
                            Text(
                                ...
                                style: const TextStyle(
                                    ...
                                    fontWeight: FontWeight.bold)),
                          Text(
                              '\u20AC${(item.product.prezzo + (item.variant?.prezzoExtra ?? 0)).toStringAsFixed(2)} x ${item.quantita}'),
                        ]),
                    trailing: IconButton(
                        icon:
              ...
              Container(
                ...
                child: Column(children: [
                  ...
                  Row(children: [
                    Expanded(
                        child: GestureDetector(
                      onTap: (_configLoaded && _ritiroAttivo)
                          ? () {
                              setState(() => _tipoConsegna = 'ritiro');
                              _showCheckoutSheet();
                            }
                          : null,
                      child: Container(
                        ...
                        decoration: BoxDecoration(
                          gradient: (_configLoaded && _ritiroAttivo)
                              ? const LinearGradient(colors: [
                                  AppTheme.primary,
                                  AppTheme.primaryDark
                                ])
                              : null,
                          color: (_configLoaded && _ritiroAttivo)
                              ? null
                              : Colors.grey.shade200,
                          borderRadius: BorderRadius.circular(16),
                          boxShadow: (_configLoaded && _ritiroAttivo)
                              ? [
                                  BoxShadow(
                                      color: AppTheme.primary
                    ...
                    Expanded(
                        child: GestureDetector(
                      onTap: (_configLoaded && _spedizioneAttiva)
                          ? () {
                              setState(() => _tipoConsegna = 'spedizione');
                              _showCheckoutSheet();
                            }
                          : null,
                      child: Container(
                        padding: const EdgeInsets.symmetric(
                            vertical: 16, horizontal: 10),
                        decoration: BoxDecoration(
                          gradient: (_configLoaded && _spedizioneAttiva)
                              ? const LinearGradient(
                                  colors: [Colors.indigo, Colors.deepPurple])
                              : null,
                          color: (_configLoaded && _spedizioneAttiva)
                              ? null
                              : Colors.grey.shade200,
                          borderRadius: BorderRadius.circular(16),
                          boxShadow: (_configLoaded && _spedizioneAttiva)
                              ? [
                                  BoxShadow(
                                      color:
